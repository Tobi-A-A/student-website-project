...
function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("storage", refreshLocalWorkspace);
    const timer = setInterval(refreshLocalWorkspace, 1000);
    return () => { window.removeEventListener("storage", refreshLocalWorkspace); clearInterval(timer); };
  }, []);
  ...
  return (
    <div className="app-shell" style={{ "--accent": theme.accent, "--paper": theme.background, "--ink": theme.ink, "--portal-font": theme.font }}>
      ...
      <main className="content">
        ...
        {active === "CSV uploads" && <CsvUploads setNotice={setNotice} />}
...
function CsvUploads({ setNotice }) {
  const [file, setFile] = useState(null);
  return <section className="panel upload-panel"><p className="eyebrow">Main admin workspace</p><h3>Bulk CSV marks upload</h3><p className="muted">Upload a CSV with studentId, assessmentId and mark columns. Validation runs before anything is imported.</p><label className="file-drop">Choose marks CSV<input type="file" accept=".csv,text/csv" onChange={(e) => setFile(e.target.files[0])} /></label>{file && <p className="muted">{file.name} ready for validation.</p>}<button className="primary" disabled={!file} onClick={() => setNotice(`“${file.name}” passed to the local validation workflow. Invalid batches are rejected.`)}>Validate and upload</button></section>;
}
